...
class NodeGCN_ba_community(torch.nn.Module):
    """
    A graph clasification model for nodes decribed in https://arxiv.org/abs/1903.03894.
    This model consists of 3 stacked GCN layers followed by a linear layer.
    """
    def __init__(self, num_features, num_classes):
        super(NodeGCN_ba_community, self).__init__()
        self.embedding_size = 200 * 4
        self.conv1 = GCNConv(num_features, 200)
        self.relu1 = ReLU()
        self.conv2 = GCNConv(200, 200)
        self.relu2 = ReLU()
        self.conv3 = GCNConv(200, 200)
        self.relu3 = ReLU()
        self.conv4 = GCNConv(200, 200)
        self.relu4 = ReLU()
        self.lin = Linear(4*200, num_classes)
...
class NodeGCN_softmax(torch.nn.Module):
    """
    A graph clasification model for nodes decribed in https://arxiv.org/abs/1903.03894.
    This model consists of 3 stacked GCN layers followed by a linear layer.
    """
    ...
    def forward(self, x, edge_index, edge_weights=None):
        ...
        # ********************************************
        final = torch.nn.Softmax(final, dim=1)
        # ********************************************
